...
/**
 * appear_animate(ref名（内部做了ref数组或单个ref判断），作者提供的动画函数名)
 */
export const appear_animate = {
  mounted() {},
  methods: {
    Animats(el, obj = {}, percentage = null) {
      ...
      if (percentage == null) {
        ...
        if (x.includes('px')) {
          ...
          el.style.transform = `translateX(${x -
            percentage * x}%) translateY(${y - percentage * y}%)`;
        } else if (x.includes('%')) {
          x = x.replace('%', '');
          el.style.transform = `translateX(${x -
            percentage * x}%) translateY(${y - percentage * y}%)`;
    ...
    $lybS2(obj, animates) {
      ...
      function lyb(item, animate, time) {
        ...
        if (entryHeight + father.offsetTop >= 0) {
          ...
          if (scrollFllow) {
            ...
            animate(item, animates, percentage);
            return;
          }
          // 进入后播放动画归位
      ...
      el.forEach(item => {
        if (this.$refs[item].length) {
          ...
          lyb(this.$refs[item], animate, time);
        }
      });
    },
  },
